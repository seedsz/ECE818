...
class RRT:
    ...
    def __init__(self,
        ...
        #self.fig = plt.figure()                                     #this made it so the figure was only created once by putting it here
        #self.set_ax()                                               #Added this
    ...
    def steer( self, from_node, to_node, extend_length=float("inf")):
        ...
        for _ in range(n_expand):
            ...
            new_node.y += self.path_resolution * math.sin(theta) * math.sin(phi)  #need both angles a line that can roate in any direction
            new_node.z += self.path_resolution * math.cos(phi)  #no way thats right
    ...
    def draw_graph(self, rnd=None):     #TODO
        ...
        self.ax.plot3D(self.start.x, self.start.y, self.start.z, "xb")  #need z
        self.ax.plot3D(self.end.x, self.end.y, self.end.z, "xg")      #need z
        plt.pause(0.01)
        return self.ax

    @staticmethod
    def plot_sphere(x, y, z, size, ax, color="-b"):
...
def main(gx=6.0, gy=10.0, gz= 9.0):
    ...
    if path is None:
        ...
        if show_animation:
            rrt.draw_graph()
            rrt.ax.plot3D([x for (x, y, z) in path], [y for (x, y, z) in path], [z for (x, y, z) in path], '-r')  #need z I think this worked with out the 3D on plot
